...
import { createPurchaseSchema, type CreatePurchaseInput } from "@/lib/schemas/order";
...
/**
 * Lõi tạo phiếu nhập (nhận hàng ngay): cộng kho, cập nhật giá vốn, ghi nợ NCC, chi quỹ.
 * Dùng chung web action + API mobile. Bản sao logic createPurchase (không "use server").
 */
export async function createPurchaseForUser(
  ...
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      for (const i of v.items) {
        await tx.insert(stockLevels).values({
          productId: i.productId, warehouseId: v.warehouseId, quantity: toQty(i.quantity),
        }).onConflictDoUpdate({
          target: [stockLevels.productId, stockLevels.warehouseId],
          set: { quantity: sql`${stockLevels.quantity} + ${toQty(i.quantity)}`, updatedAt: sql`now()` },
        });

        await tx.insert(stockMovements).values({
          productId: i.productId, warehouseId: v.warehouseId, type: "purchase",
          quantity: toQty(i.quantity), unitCost: toMoney(i.unitCost),
          refType: "purchase", refId: po.id, note: po.code, createdBy: profileId,
        });

        const netUnit = i.quantity > 0 ? lineTotal(i) / i.quantity : i.unitCost;
        await tx.update(products).set({
          costPrice: toMoney(netUnit), lastPurchasePrice: toMoney(i.unitCost), updatedAt: sql`now()`,
      ...
      await tx.update(products).set({ supplierId: v.supplierId })
    ...
    console.error("createPurchase (core) failed:", e);
