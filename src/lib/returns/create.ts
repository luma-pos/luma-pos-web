...
import { createReturnSchema, type CreateReturnInput } from "@/lib/schemas/returns";
...
/** Lõi trả hàng theo hóa đơn (web + API mobile). Bản sao logic createReturn. */
  ...
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      const prevReturned = await tx
        .select({ orderItemId: returnItems.orderItemId, qty: sql<string>`coalesce(sum(${returnItems.quantity}), 0)` })
      ...
      for (const i of v.items) {
        ...
        rows.push({
          returnId: "", orderItemId: src.id, productId: src.productId, productName: src.productName,
          unitName: src.unitName, unitMultiplier: src.unitMultiplier, quantity: toQty(i.quantity),
          unitPrice: src.unitPrice, total: toMoney(lineRefund), restock: i.restock,
        });
      }
      ...
      const [ret] = await tx.insert(returns).values({
        code: generateCode("TH"), orderId: order.id, customerId: order.customerId,
        warehouseId: order.warehouseId, reason: v.reason, refundMethod: v.refundMethod,
        totalRefund: toMoney(totalRefund), note: v.note || null, createdBy: profileId,
      ...
      if (order.warehouseId) {
        for (const r of rows.filter((x) => x.restock)) {
          ...
          await tx.insert(stockMovements).values({
            productId: r.productId, warehouseId: order.warehouseId, type: "return_in",
            quantity: toQty(baseQty), refType: "return", refId: ret.id,
            note: `${ret.code} ← ${order.code}`, createdBy: profileId,
          });
        }
      }
      ...
      const allReturned = await tx
        ...
        .where(eq(orderItems.orderId, order.id)).groupBy(returnItems.orderItemId);
    ...
    console.error("createReturn (core) failed:", e);
