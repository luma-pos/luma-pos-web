...
import { StocktakeRowActions } from "./stocktake-actions";
              ...
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.id} className={cn("hover:bg-slate-50 dark:hover:bg-slate-800/40", r.status === "cancelled" && "opacity-60")}>
                    ...
                    <td className="px-4 py-3">
                      <span className={cn("inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium", STATUS_STYLES[r.status])}>
                        {t(`stocktakes.status.${r.status}` as never)}
                      </span>
                    </td>
