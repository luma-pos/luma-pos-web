...
import { CashTxForm } from "./cash-tx-form";

interface PageProps {
  searchParams: Promise<{ fund?: string; type?: string; page?: string; size?: string }>;
}
...
export default async function CashbookPage({ searchParams }: PageProps) {
  ...
  const href = (overrides: Record<string, string | undefined>) => {
    ...
    const merged: Record<string, string | undefined> = { fund: params.fund, type: params.type, ...overrides };
    for (const [k, v] of Object.entries(merged)) if (v) sp.set(k, v);
    const s = sp.toString();
    return `${Routes.Cashbook}${s ? `?${s}` : ""}`;
  };
                ...
                <div key={r.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-slate-500 mt-0.5">{formatDate(r.createdAt)} · {r.fund === "cash" ? t("cashbook.fundCash") : t("cashbook.fundBank")}</div>
                    </div>
                    {amount}
                  </div>
                  <div className="flex items-center gap-2 mt-2">
                    <span className={cn("inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium", CAT_STYLES[r.category] ?? CAT_STYLES.other)}>
                      {t(`cashbook.categories.${r.category}` as never)}
                    </span>
                    <span className="text-xs text-slate-500 truncate">
                      {r.refType === "order" && r.refId
                        ? <Link href={Routes.order(r.refId)} className="text-primary-600 hover:underline">{r.note}</Link>
                        : r.note ?? ""}
                    </span>
                  </div>
                </div>
